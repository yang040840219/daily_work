...
public class PaginationInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (obj instanceof Map) {
			...
			if (parameterMap.containsKey(Pagination.MAP_PAGE_FIELD)) {
				page = (Pagination<?>) parameterMap.get(Pagination.MAP_PAGE_FIELD);
				isPage = true;
			}
		}
		...
		if (isPage) {
			...
			// 给当前的page参数对象设置总记录数
			this.setTotalRecord((Map) obj, mappedStatement, connection);

			// 计算分页变量
			page.init(page.getTotalCount(), page.getPageSize(), page.getCurrentPage());
	...
	/**
	 * 根据page对象获取对应的分页查询Sql语句，这里只做了两种数据库类型，Mysql
	 * 
	 * @param page
	 *            分页对象
	 * @param sql
	 ...
	 *            当前的数据库连接
	 */
	private void setTotalRecord(Map paramMap, MappedStatement mappedStatement, Connection connection) {
		Pagination<?> page = (Pagination)paramMap.get(Pagination.MAP_PAGE_FIELD) ;
	...
	private String getCountSql(String sql) {
		...
		return "select count(1) from (" + sql + ") as _a";
	}
